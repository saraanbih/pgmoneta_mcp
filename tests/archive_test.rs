...
use pgmoneta_mcp::handler::archive::{ArchiveRequest, ArchiveTool};
...
fn assert_success_archive_response(response: &str) -> Value {
...
async fn archive_with_current_primary_test() {
    common::init_config();
    ...
    let response = ArchiveTool::invoke(&handler, request)
        .await
        .expect("Archive should succeed");

    let request = assert_success_archive_response(&response);
...
#[serial]
#[ignore = "requires pgmoneta stack (see test/check.sh and full-test CI job)"]
async fn archive_with_primary_action_timeline_test() {
    common::init_config();
    let _guard = common::backup_fixture_lock().await;
    common::ensure_backup("primary")
        .await
...
#[serial]
#[ignore = "requires pgmoneta stack (see test/check.sh and full-test CI job)"]
async fn archive_with_primary_replica_test() {
    common::init_config();
    let _guard = common::backup_fixture_lock().await;
    common::ensure_backup("primary")
        .await
